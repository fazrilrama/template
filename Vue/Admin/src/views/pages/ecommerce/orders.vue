  ...
  <Layout>
    ...
    <BRow>
      <BCol cols="12">
        <div>
          <div class="float-end">
            <BForm class="d-inline-flex mb-3">
          ...
          <BButton variant="success" type="button" class="mb-3">
            <i class="mdi mdi-plus me-1"></i> Add New Order
          </BButton>
        </div>
        <BTableSimple class="table-centered datatable dt-responsive nowrap table-card-list dataTable no-footer dtr-inline">
          <BRow>
          ...
          <BTable table-class="table table-centered datatable table-card-list" thead-tr-class="bg-transparent" :items="orderData" :fields="fields" responsive="sm" :per-page="perPage" :current-page="currentPage" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :filter="filter" :filter-included-fields="filterOn" @filtered="onFiltered">
            ...
            <template v-slot:cell(id)="data">
              <BLink href="javascript: void(0);" class="text-dark fw-bold">{{ data.item.id }}</BLink>
            </template>
            ...
            <template v-slot:cell(status)="data">
              <div class="badge bg-pill bg-soft-success font-size-12" :class="{
